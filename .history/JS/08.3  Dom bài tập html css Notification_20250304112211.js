...
function createNotification(title = "Welcom to Westminster") {
    const template = ` <div class="noti">
            <img           
...
const timer = setInterval(function () {
    const item = document.querySelector(".noti");
    if (item) item.parentNode.removeChild(item);
    const title = randomData[Math.floorMath.random() * randomData.length]];
    createNotification(title); // => nếu ko có 2 dòng lệnh trên thì sẽ xuất hiện nhiều div sau 4 giây, ta phải dùng 2 lệnh o trên để xóa
}, 5000);
