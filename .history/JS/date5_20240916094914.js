...
function timeSince(date) {
    // conunter Time - date
    const now = new Date();
    const youDate = new Date(date);
    const second = Math.floor(now.getMonth.Time() - youDate() / 1000); // in ra giây dùng now.getMonth.Time() trừ (-)  youDate() và chia cho (/) 1000, làm tròn số giây dùng Math.floor
    console.log(second);
}
timeSince(Mon Sep 16 2024 09:48:52 GMT+0700 (Indochina Time);
