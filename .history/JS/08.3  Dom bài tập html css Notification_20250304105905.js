...
const randomData = [" Welcom to Notification Javascript"];

const timer = setInterval(function () {
    const item = document.querySelector(".noti");
    if (item) item.parentNode.removeChild(item);
    createNotification("Welcom Notification"); // => nếu ko có 2 dòng lệnh trên thì sẽ xuất hiện nhiều div sau 4 giây, ta phải dùng 2 lệnh o trên để xóa
}, 8000);
