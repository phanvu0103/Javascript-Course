...
//
// *** *** *** *** ***
//Bài 86: Bài 85: Bài tập chương 6 số 1 và 2
...
function reverseNumber(number10) {
    ...
    const value = number10.toString().split("").reverse().join("");
    console.log(value);
}
reverseNumber(1234);

console.log("--------- ***  Bài tập 2 số 3  *** ---------");
function reverseNumber(number10) {
    // convert to String -> đổi sang String
    // split("") đổi thành từng ký tự ra thành mảng array[value]
    // reverse() đảo ngược chiều lại từng mảng
    // join("") thành số ban đầu,, nhưng nó vẫn lài String
    // bỏ 4 lệnh trên vào () để đổi thành số
    const value = number10.toString().split("").reverse().join("")*Math.sign(number10);
    console.log(value);
}
reverseNumber(-567);